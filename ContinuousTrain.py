### CLIENT USAGE PREDICTOR - continuous training script
### Code produced by Lang (Ron) Chen August-October 2021 for Lucidity Software
""" Wrangles raw data of recent weeks, appends it to the previous wrangled data and outputs predictor objects for predicting the client usage trend for the upcoming week """
...
# Output: several partial outputs - partial outputs of wrangled data and important coefficietns, tree objects (.dot), DecisionTreePredictor objects (.pickle), Statistics.csv, exported to the History directory (into the relevent week) and potentially the home directory (if the new predictor is good enough to replace the current predictor)
...
# If has already run Predict.py with same arguments then this PART 2 and PART 3 do not need to be rerun
if not os.path.isfile(f'./History/Week {thisweek}/_3_combined_cleaned_data.csv'):
    ...
    def wrangle(filename, datedata, mode):
        ...
        # export the wrangled file as a csv
...
# reread data
data = pd.read_csv(f'./History/Week {thisweek}/Partial_Output/_3_combined_cleaned_data.csv')
coef = pd.read_csv(f'./History/Week {thisweek}/Partial_Output/_4_UsageScoreCoefficients.csv')
...
#### FUTURE CHANGE: If one day the files are so large that the time required for this function is large, then consider splitting the distb function into 2 functions (but then need to re-input the variables)
...
# Slightly different from PART 11 of InitialTraining.py because it also needs to compare statistics of the new predictor (after including data of this week) with that of the one currently in use, and if the new predictor has better statistics then it will replace/topple the current one

# read in statistics of the predictor which is currently in use
prev_statistics_DF = pd.read_csv('Statistics.csv')
prev_statistics = prev_statistics_DF['Statistics']

# write function to help with returning detailed accuracy score
def as_pro2(y_test, y_pred, name):
    """ Accuracy Score Pro - version 2 - more detailed than as_pro but purely used to output statistics """
    y_test = list(y_test[name])
    y_pred = list(y_pred)
...
def train(p, q1, q3, tag, data, selection, prevstats, thisweek):
    ...
    # decide whether this trainer should replace the previous trainer by comparing the two's true pos 1 + true pos 2 - bad false pos 1 - bad false pos 2
    #### FUTURE UPDATE: this decision method is not perfect because there is some inherent varience with the scores that are related to the test-train split. Could try use statistic methods to create a confidence interval and only topple if new_accuracy_score is outside the 95% CI of old_accuracy_score
    topple = 0
    old_accuracy_score = prevstats[0] + prevstats[1] - prevstats[2] - prevstats[3]
    new_accuracy_score = stat[0] + stat[1] - stat[2] - stat[3]
    
    if new_accuracy_score > old_accuracy_score:
        topple = 1
    ...
    return dtbot, dtmid, dttop, topple

# make the final predictor
if selection == 1:
    dtbot, dtmid, dttop, topple = train(p1, q1_1, q3_1, tag1, data, selection, prev_statistics, thisweek)
elif selection == 2:
    dtbot, dtmid, dttop, topple = train(p2, q1_2, q3_2, tag2, data, selection, prev_statistics, thisweek)
else:
    dtbot, dtmid, dttop, topple = train(p3, q1_3, q3_3, tag3, data, selection, prev_statistics, thisweek)
